...
class CI_DB_driver {
	...
	var $query_count	= 0;
	var $bind_marker	= '?';
	var $save_queries	= FALSE; //radu: FUCKING MEMORY LEAK!!!!
	var $queries		= array();
	...
	function initialize($create_db = FALSE)
	{
		...
		if ( ! $this->conn_id )
		{
			log_message('Unable to connect to the database');
		...
		if ($this->database != '')
		{
			if ( ! $this->db_select())
			{
				...
				if ($create_db == TRUE)
				{
					...
					{
						log_message('Unable to create database: '.$this->database);
					...
					else
					{
						...
						if ($this->db_select())
						{
							...
							{
								log_message('Unable to set database connection charset: '.$this->char_set);
				...
				log_message('Unable to select database: '.$this->database);
	...
	function query($sql, $binds = FALSE, $return_object = TRUE)
	{
		...
		if (FALSE === ($this->result_id = $this->simple_query($sql)))
		{
			...
			if ($this->db_debug)
			{
				log_message('Query error: '.$this->_error_message());
	...
	{
		// radu if( DEBUG_MODE or ($this->is_write_type($sql) === TRUE) )
		log_message( "[SQL] ".$sql );
	...
	function display_error($error = '', $swap = '', $native = FALSE)
	{
		...
		{
			$message = ( ! is_array($error)) ? array(str_replace('%s', $swap, $lang[$error])) : $error;
		}
